...
function renderReadme(state: AgentMemoryState): string {
  ...
  return [
    "# Agent Memory",
    "",
    "This directory is a generated, human-readable projection of the canonical repository memory state.",
    "",
    "## Canonical Source",
    "",
    ...
    "## Refresh Flow",
    "",
    "- Run `npx agent-memory update` to refresh the canonical state and all projections.",
    "- Run `npx agent-memory update --validate` to refresh with a real validation baseline.",
    "- Run `npx agent-memory validate` to audit whether state, projections, entry wiring, and validation freshness still line up.",
    "",
    "## What Lives Here",
    "",
    "- `project-map.md`: stable structure, modules, entrypoints, and architecture notes.",
    "- `current-focus.md`: the current operating picture, known risks, and validation snapshot.",
    "- `gotchas.md`: costly traps that are easy to forget and expensive to rediscover.",
    "- `next-steps.md`: a short list of concrete starting points for the next contributor.",
  ].join("\n");
}
...
export function buildEntrySnippet(state: AgentMemoryState): string {
  return [
    ...
    "",
    "Human-readable projections live in `docs/agent-memory/`.",
    "",
    ...
    "",
    "Refresh the canonical memory with `npx agent-memory update` and audit it with `npx agent-memory validate`.",
